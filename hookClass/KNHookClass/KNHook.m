...
+(void)hookClass:(NSString*)hookString
{
    ...
    IMP forwardInvocationImpl = imp_implementationWithBlock(^(id object, NSInvocation *invocation) {
        ...
        NSString *newSelectorName = [KNHook methodLogMethodName:NSStringFromSelector(invocation.selector)];
        ...
        @try {
            ...
            newSelectorName=[newSelectorName stringByReplacingOccurrencesOfString:KN withString:@""];
            ...
            id obj =[KNHook getReturnValueInvocatein:invocation];
            NSString *retObj=[NSString stringWithFormat:@"%@",obj];
            NSMutableString *muString=[[NSMutableString alloc] init];
            for (long i=2; i<number; i++) {
                
                NSObject *ret = [KNHook argumentAtIndex:i withInvocatein:invocation];
    ...
    if (![superClassString isEqualToString:hookString]) {
        if (!class_isMetaClass(hookClass)) {
            [KNHook hookClass:superClassString];
        }
    }
}
...
+ (NSString *)methodLogMethodName:(NSString *)selName {
    return [KN stringByAppendingFormat:@"%@",selName];
}
