...
/**
 追踪方法调用顺序(deep)
 @param aClass 追踪的class
 */
+ (void)logMethodWithClass:(NSString*)aClassstr{
    
    [ANYMethodLog logMethodWithClass:NSClassFromString(aClassstr) condition:^BOOL(SEL sel) {
        return  YES;
        ...
        //2、selectorString : selector argvalue
        ...
        NSLog(@"《before》方法调用deep:%@\n[target:%@\n《selector》: %@]\n", deepString , target, selectorString);
        ...
        //        NSLog(@"方法调用前deep:%@\n[##########target:%@\n##########selector: %@]", deepString , target, selectorString);
        NSLog(@"《after》方法调用deep:%@\n[target:%@\n《selector》: %@]\n返回值:%@\ninterval:%f", deepString , target, selectorString,retValue,interval);
